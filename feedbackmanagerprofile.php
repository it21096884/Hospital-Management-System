...
<html>
	<head>
		...
		<link rel="stylesheet" href="Styles/staffProfile.css">
		...
		<div class = "background">
			...
			<div class="box">   
				<h2 class = "detailsheading" > 
					<?php			
						$uID = $_SESSION["uID"];
						$sql = "SELECT * FROM feedback_manager WHERE Feedback_ManagerID = '$uID'";
						$result = $con->query($sql);
						
						if(mysqli_num_rows($result) > 0){
							while( $row = mysqli_fetch_assoc($result) ){
								echo "Feedback Manager : ". $row["First_Name"]. " ". $row["Last_Name"];
							}
						}
					?>
				</h2> 
				...
				<p class="dashboardlist"> &nbsp &nbsp 					
					<?php			
						...
						if(mysqli_num_rows($result) > 0){
							while( $row = mysqli_fetch_assoc($result) ){
								echo "ID : ". $row["Feedback_ManagerID"];
							}
						}
					?> 
				</p>
